...
# -*- coding: utf-8 -*-
"""
Testbench modificado para señales senoidales con parámetros aleatorios
"""


#%%  Generación de señales de prueba
def my_testbench_random_seno():
    ...
    tt = np.linspace(0, (N-1)*ts, N).flatten() #convierte cualquier array de cualquier dimensión en un array unidimensional (1D).
    
    # Genera 4 numeros aleatorios para cada parametro
    freqs = np.random.uniform(1, 20, 4)       #  entre 1 y 20 Hz
    amps  = np.random.uniform(0.5, 2, 4)      #entre 0.5 y 2
    fases = np.random.uniform(0, 2*np.pi, 4)  #entre 0 y 2pi
    
    # Creamos un array vacío para almacenar las señales
    x = np.zeros((N,0))
    
    # Generamos las 4 señales
    for ii in range(4):
        aux = amps[ii] * np.sin(2*np.pi*freqs[ii]*tt + fases[ii])
        x = np.hstack([x, aux.reshape(N,1)]) #hstack concatena arrays horizontalmente.
    
    # Descripciones para la leyenda
    descripcion = [f'{freqs[ii]:.2f} Hz, A={amps[ii]:.2f}, Fases={fases[ii]:.2f}' for ii in range(4)]
    

#%%  Presentación de resultados
    plt.figure(figsize=(10,5))
    line_hdls = plt.plot(tt, x)
    plt.title('Señales Senoidales Aleatorias')
    plt.xlabel('Tiempo [s]')
    plt.ylabel('Amplitud [V]')
    axes_hdl = plt.gca()
    axes_hdl.legend(line_hdls, descripcion, loc='upper right')
    plt.show()

#%% Ejecutamos la función
my_testbench_random_seno()
